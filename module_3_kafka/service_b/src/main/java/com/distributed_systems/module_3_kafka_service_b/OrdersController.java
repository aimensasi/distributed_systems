package com.distributed_systems.module_3_kafka_service_b;

import com.google.gson.Gson;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.jdbc.support.GeneratedKeyHolder;
import org.springframework.jdbc.support.KeyHolder;
import org.springframework.kafka.annotation.KafkaListener;
import org.springframework.kafka.core.KafkaTemplate;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import java.sql.PreparedStatement;
import java.sql.Statement;
import java.util.Map;

@RestController
@RequestMapping("lab35/orders")
public class OrdersController {


  private final KafkaTemplate<String, String> kafkaTemplate;
  private final JdbcTemplate jdbcTemplate;

  public OrdersController(KafkaTemplate<String, String> kafkaTemplate, JdbcTemplate jdbcTemplate) {
    this.jdbcTemplate = jdbcTemplate;
    this.kafkaTemplate = kafkaTemplate;
  }


  @GetMapping("{userId}/{productId}/{quantity}")
  public String create(
    @PathVariable("userId") int userId,
    @PathVariable("productId") int productId,
    @PathVariable("quantity") int quantity)
  {
    KeyHolder keyHolder = new GeneratedKeyHolder();

    jdbcTemplate.update(connection -> {
      PreparedStatement ps = connection.prepareStatement(
        "INSERT INTO orders (userId, productId, quantity) VALUES (?, ?, ?)",
        Statement.RETURN_GENERATED_KEYS // Inform JDBC to return keys
      );
      ps.setLong(1, userId);
      ps.setLong(2, productId);
      ps.setInt(3, quantity);
      return ps;
    }, keyHolder);

    int newOrderId = (int) keyHolder.getKeys().get("id");

    String payload = new Gson().toJson(
      Map.of("orderId", newOrderId, "productId", productId, "quantity", quantity, "userId", userId)
    );
    kafkaTemplate.send("ORDER_CREATED", String.valueOf(newOrderId), payload);
    return "Done";
  }

  @KafkaListener(topics = "PAYMENT_COMPLETED", id = "order-payment-completed")
  public void processCompletedOrder(String eventPayload) {
    Map<String, Object> result = new Gson().fromJson(eventPayload, Map.class);
    int orderId = ((Number) result.get("orderId")).intValue();
    int productId = ((Number) result.get("productId")).intValue();
    int quantity = ((Number) result.get("quantity")).intValue();

    System.out.printf("Processing Order %s, for %s with quantity of \n", orderId, productId, quantity);

    jdbcTemplate.update(
      "UPDATE orders SET status = ? WHERE id = ?",
      "COMPLETED",
      orderId
    );

    kafkaTemplate.send("ORDER_COMPLETED", String.valueOf(orderId), eventPayload);
  }

  @KafkaListener(topics = "INVENTORY_OUT_OF_STOCK", id = "order-inventory-outofstock")
  public void handleOutOfStock(String eventPayload) {
    Map<String, Object> result = new Gson().fromJson(eventPayload, Map.class);
    int orderId = ((Number) result.get("orderId")).intValue();
    int productId = ((Number) result.get("productId")).intValue();
    int quantity = ((Number) result.get("quantity")).intValue();

    System.out.printf("Processing Order %s, for %s with quantity of \n", orderId, productId, quantity);

    jdbcTemplate.update(
      "UPDATE orders SET status = ? WHERE id = ?",
      "FAILED",
      orderId
    );

    kafkaTemplate.send("ORDER_FAILED", String.valueOf(orderId), eventPayload);
  }

  @KafkaListener(topics = "PAYMENT_FAILED", id = "order-payment-failed")
  public void handlePaymentFailed(String eventPayload) {
    Map<String, Object> result = new Gson().fromJson(eventPayload, Map.class);
    int orderId = ((Number) result.get("orderId")).intValue();
    int productId = ((Number) result.get("productId")).intValue();
    int quantity = ((Number) result.get("quantity")).intValue();

    System.out.printf("Processing Order %s, for %s with quantity of \n", orderId, productId, quantity);

    // make failed with reason of payment failed
    jdbcTemplate.update(
      "UPDATE orders SET status = ? WHERE id = ?",
      "FAILED",
      orderId
    );

    kafkaTemplate.send("ORDER_FAILED", String.valueOf(orderId), eventPayload);
  }
}
